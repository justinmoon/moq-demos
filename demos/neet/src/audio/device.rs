...
pub fn find_input_stream_config(
    ...
    let config = if !supported_configs.is_empty() {
        ...
        config_range
            .try_with_sample_rate(format.sample_rate)
...
fn cmp_stream_format(
    ...
    let is_perfect = |x: &SupportedStreamConfigRange| {
        x.channels() == format.channel_count
            && x.sample_format() == SampleFormat::F32
            && x.try_with_sample_rate(format.sample_rate).is_some()
    };
    ...
    let cmp_channel_count =
        (a.channels() == format.channel_count).cmp(&(b.channels() == format.channel_count));
    ...
    let cmp_sample_rate = (a.try_with_sample_rate(format.sample_rate).is_some())
        .cmp(&(b.try_with_sample_rate(format.sample_rate).is_some()));
